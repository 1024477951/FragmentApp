package cn.jiguang.imui.chatinput.camera.ui;
...
import cn.jiguang.imui.chatinput.camera.DataHolder;
import cn.jiguang.imui.chatinput.camera.ImageDataSource;
import cn.jiguang.imui.chatinput.camera.ImagePicker;
import cn.jiguang.imui.chatinput.camera.adapter.ImageFolderAdapter;
import cn.jiguang.imui.chatinput.camera.adapter.ImageRecyclerAdapter;
import cn.jiguang.imui.chatinput.camera.bean.ImageFolder;
import cn.jiguang.imui.chatinput.camera.bean.ImageItem;
import cn.jiguang.imui.chatinput.camera.util.Utils;
import cn.jiguang.imui.chatinput.camera.view.FolderPopUpWindow;
import cn.jiguang.imui.chatinput.camera.view.GridSpacingItemDecoration;
...
public class ImageGridActivity extends ImageBaseActivity implements ImageDataSource.OnImagesLoadedListener, ImageRecyclerAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null) {
            mSelectedImages= (ArrayList<ImageItem>) getIntent().getSerializableExtra(ImagePicker.EXTRA_RESULT_ITEMS);
        }
...
//        mImageGridAdapter = new ImageGridAdapter(this, null);
        ...
        if (mSelectedImages != null) {
            if (mSelectedImages.size()>0){
                mRecyclerAdapter.getImagePicker().clearSelectedImages();
                mRecyclerAdapter.getImagePicker().setSelectedImages(mSelectedImages);
                mRecyclerAdapter.setSelectedImages(mRecyclerAdapter.getImagePicker().getSelectedImages());
            }
        }
